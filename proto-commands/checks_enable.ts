...
export const Checks_EnableRequest = {
  ...
  decode(input: _m0.Reader | Uint8Array, length?: number): Checks_EnableRequest {
    const reader = input instanceof _m0.Reader ? input : _m0.Reader.create(input);
    let end = length === undefined ? reader.len : reader.pos + length;
  ...
  create<I extends Exact<DeepPartial<Checks_EnableRequest>, I>>(base?: I): Checks_EnableRequest {
    return Checks_EnableRequest.fromPartial(base ?? ({} as any));
  },
...
export interface ChecksEnableService {
  execute(request: Checks_EnableRequest): Promise<Response>;
}

export const ChecksEnableServiceServiceName = "liquibase.pro.ChecksEnableService";
export class ChecksEnableServiceClientImpl implements ChecksEnableService {
  private readonly rpc: Rpc;
  private readonly service: string;
  constructor(rpc: Rpc, opts?: { service?: string }) {
    this.service = opts?.service || ChecksEnableServiceServiceName;
    this.rpc = rpc;
    this.execute = this.execute.bind(this);
  }
  execute(request: Checks_EnableRequest): Promise<Response> {
    const data = Checks_EnableRequest.encode(request).finish();
