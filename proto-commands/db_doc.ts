...
function createBaseDbDocRequest(): DbDocRequest {
  return {
    changelogFile: "",
...
export const DbDocRequest = {
  encode(message: DbDocRequest, writer: _m0.Writer = _m0.Writer.create()): _m0.Writer {
    ...
    if (message.outputDirectory !== "") {
      writer.uint32(66).string(message.outputDirectory);
    }
    if (message.password !== undefined) {
      writer.uint32(74).string(message.password);
    }
    if (message.schemas !== undefined) {
      writer.uint32(82).string(message.schemas);
    }
    if (message.url !== "") {
  ...
  decode(input: _m0.Reader | Uint8Array, length?: number): DbDocRequest {
    const reader = input instanceof _m0.Reader ? input : _m0.Reader.create(input);
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = createBaseDbDocRequest();
  ...
  fromJSON(object: any): DbDocRequest {
    return {
      ...
      outputDirectory: isSet(object.outputDirectory) ? globalThis.String(object.outputDirectory) : "",
  ...
  toJSON(message: DbDocRequest): unknown {
    const obj: any = {};
    if (message.changelogFile !== "") {
  ...
  create<I extends Exact<DeepPartial<DbDocRequest>, I>>(base?: I): DbDocRequest {
    return DbDocRequest.fromPartial(base ?? ({} as any));
  },
  fromPartial<I extends Exact<DeepPartial<DbDocRequest>, I>>(object: I): DbDocRequest {
    const message = createBaseDbDocRequest();
...
export interface DbDocService {
  execute(request: DbDocRequest): Promise<Response>;
}

export const DbDocServiceServiceName = "liquibase.DbDocService";
export class DbDocServiceClientImpl implements DbDocService {
  private readonly rpc: Rpc;
  private readonly service: string;
  constructor(rpc: Rpc, opts?: { service?: string }) {
    this.service = opts?.service || DbDocServiceServiceName;
    this.rpc = rpc;
    this.execute = this.execute.bind(this);
  }
  execute(request: DbDocRequest): Promise<Response> {
    const data = DbDocRequest.encode(request).finish();
