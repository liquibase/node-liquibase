...
export interface FutureRollbackFromTagSqlRequest {
  ...
  password?: string;
  /** required* Tag ID to rollback from */
  tag: string;
...
function createBaseFutureRollbackFromTagSqlRequest(): FutureRollbackFromTagSqlRequest {
  return {
...
export const FutureRollbackFromTagSqlRequest = {
  encode(message: FutureRollbackFromTagSqlRequest, writer: _m0.Writer = _m0.Writer.create()): _m0.Writer {
    ...
    if (message.tag !== "") {
      writer.uint32(106).string(message.tag);
    }
    if (message.url !== "") {
  ...
  decode(input: _m0.Reader | Uint8Array, length?: number): FutureRollbackFromTagSqlRequest {
    const reader = input instanceof _m0.Reader ? input : _m0.Reader.create(input);
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = createBaseFutureRollbackFromTagSqlRequest();
  ...
  fromJSON(object: any): FutureRollbackFromTagSqlRequest {
    return {
  ...
  toJSON(message: FutureRollbackFromTagSqlRequest): unknown {
    const obj: any = {};
    if (message.changeExecListenerClass !== undefined) {
  ...
  create<I extends Exact<DeepPartial<FutureRollbackFromTagSqlRequest>, I>>(base?: I): FutureRollbackFromTagSqlRequest {
    return FutureRollbackFromTagSqlRequest.fromPartial(base ?? ({} as any));
  },
  fromPartial<I extends Exact<DeepPartial<FutureRollbackFromTagSqlRequest>, I>>(
    object: I,
  ): FutureRollbackFromTagSqlRequest {
    const message = createBaseFutureRollbackFromTagSqlRequest();
...
export interface FutureRollbackFromTagSqlService {
  execute(request: FutureRollbackFromTagSqlRequest): Promise<Response>;
}

export const FutureRollbackFromTagSqlServiceServiceName = "liquibase.FutureRollbackFromTagSqlService";
export class FutureRollbackFromTagSqlServiceClientImpl implements FutureRollbackFromTagSqlService {
  private readonly rpc: Rpc;
  private readonly service: string;
  constructor(rpc: Rpc, opts?: { service?: string }) {
    this.service = opts?.service || FutureRollbackFromTagSqlServiceServiceName;
    this.rpc = rpc;
    this.execute = this.execute.bind(this);
  }
  execute(request: FutureRollbackFromTagSqlRequest): Promise<Response> {
    const data = FutureRollbackFromTagSqlRequest.encode(request).finish();
