...
export const RollbackRequest = {
  encode(message: RollbackRequest, writer: _m0.Writer = _m0.Writer.create()): _m0.Writer {
    ...
    if (message.tag !== "") {
      writer.uint32(122).string(message.tag);
    }
    if (message.tagVersion !== undefined) {
      writer.uint32(130).string(message.tagVersion);
    }
    if (message.url !== "") {
...
export interface RollbackService {
  execute(request: RollbackRequest): Promise<Response>;
}

export const RollbackServiceServiceName = "liquibase.RollbackService";
export class RollbackServiceClientImpl implements RollbackService {
  private readonly rpc: Rpc;
  private readonly service: string;
  constructor(rpc: Rpc, opts?: { service?: string }) {
    this.service = opts?.service || RollbackServiceServiceName;
    this.rpc = rpc;
    this.execute = this.execute.bind(this);
  }
  execute(request: RollbackRequest): Promise<Response> {
    const data = RollbackRequest.encode(request).finish();
