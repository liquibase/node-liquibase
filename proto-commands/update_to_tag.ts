...
export interface UpdateToTagRequest {
...
function createBaseUpdateToTagRequest(): UpdateToTagRequest {
  return {
...
export const UpdateToTagRequest = {
  encode(message: UpdateToTagRequest, writer: _m0.Writer = _m0.Writer.create()): _m0.Writer {
    ...
    if (message.tag !== "") {
      writer.uint32(138).string(message.tag);
    }
    if (message.url !== "") {
  ...
  decode(input: _m0.Reader | Uint8Array, length?: number): UpdateToTagRequest {
    const reader = input instanceof _m0.Reader ? input : _m0.Reader.create(input);
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = createBaseUpdateToTagRequest();
  ...
  fromJSON(object: any): UpdateToTagRequest {
    return {
  ...
  toJSON(message: UpdateToTagRequest): unknown {
    const obj: any = {};
    if (message.changeExecListenerClass !== undefined) {
  ...
  create<I extends Exact<DeepPartial<UpdateToTagRequest>, I>>(base?: I): UpdateToTagRequest {
    return UpdateToTagRequest.fromPartial(base ?? ({} as any));
  },
  fromPartial<I extends Exact<DeepPartial<UpdateToTagRequest>, I>>(object: I): UpdateToTagRequest {
    const message = createBaseUpdateToTagRequest();
...
export interface UpdateToTagService {
  execute(request: UpdateToTagRequest): Promise<Response>;
}

export const UpdateToTagServiceServiceName = "liquibase.UpdateToTagService";
export class UpdateToTagServiceClientImpl implements UpdateToTagService {
  private readonly rpc: Rpc;
  private readonly service: string;
  constructor(rpc: Rpc, opts?: { service?: string }) {
    this.service = opts?.service || UpdateToTagServiceServiceName;
    this.rpc = rpc;
    this.execute = this.execute.bind(this);
  }
  execute(request: UpdateToTagRequest): Promise<Response> {
    const data = UpdateToTagRequest.encode(request).finish();
