...
import {
	ParamsModel,
	WithOptionalSubstitutionParams
} from './models/commands/substitution-params.model';
...
export class Liquibase {
	...
	public rollbackOneChangesetSQL(
		params: RollbackOneChangesetSQLCommandAttributes
	): Promise<string> {
	...
	private stringifyParams(
		...
		const commandAcceptsPropertyAsPositionalArgument = Object.values(
			CommandsWithPositionalArguments
	...
	private getSubstitutionArguments({
		substitutionParams
	}: WithOptionalSubstitutionParams<ParamsModel>): string {
	...
	private loadParamsFromLiquibasePropertiesFileOnDemand(
		liquibasePropertyPath?: string
	): LiquibaseConfig | undefined {
